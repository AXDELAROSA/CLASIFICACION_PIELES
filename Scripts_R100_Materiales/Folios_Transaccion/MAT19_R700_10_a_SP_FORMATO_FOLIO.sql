...
/*
 ...
 EXEC	[dbo].[PG_FORMATO_FOLIO_ENCABEZADO_PDF] 0,0, 3850955
...
AS
	...
	SELECT	@PP_FOLIO			AS FOLIO,
			CONCAT('*', CONVERT(VARCHAR(20), @PP_FOLIO), '*') 			AS FOLIO_BAR_CODE,
			@VP_COLOUR			AS COLOR,
...
/*
 ...
 EXEC	[dbo].[PG_FORMATO_FOLIO_ENCABEZADO_PDF] 0,0, 3813878
 EXEC	[dbo].[PG_FORMATO_FOLIO_DEV_EXTRA_PDF] 0,0, 3879281
 EXEC	[dbo].[PG_FORMATO_FOLIO_DEV_EXTRA_PDF] 0,0, 3813669
 EXEC	[dbo].[PG_FORMATO_FOLIO_DETALLE_PDF]	0,0, 3793491
*/
...
AS
	...
	-- //////SE OBTIENE LA LOCACION Y EL TIPO DEL FOLIO////////////////////////////////////////////////////////
	DECLARE @VP_LOCACION VARCHAR(5) = '', @VP_TIPO VARCHAR(5) = '', @VP_ORDEN VARCHAR(10) = '';

	SELECT	@VP_TIPO = ISNULL(STATUS, ''),
			@VP_LOCACION = ISNULL(MACHINE, ''),
			@VP_ORDEN = ISNULL(JOBNO, '') 
	FROM RP_Folios (NOLOCK)
	WHERE TAG = @PP_FOLIO
	...
	IF @VP_TIPO <> 'B'
		BEGIN
			...
			-- /////////SI EL FOLIO ESTA EN MESA CARGA MOVIMIENTOS DE EXTRAS Y DEVOLUCIONES/////////////////////////////////////////////////////
			IF SUBSTRING(@VP_LOCACION, 1,1) IN ('T', 'G')
				BEGIN
					-- ////SE OBTIENEN LAS PIELES CARGADAS EN EL FOLIO Y SE PONE * A QUE YA TIENEN CORTE//////////////////////////////////////////////////////////
					INSERT INTO @VP_TBL_FOLIO
					SELECT	'ACTUAL',
							LTRIM(RTRIM(LOT))	AS LOTE, 
							-- ===========================
							--(	CASE	WHEN HIDENO IS NULL THEN '  ' + HIDE
							--			ELSE  '* ' + HIDE  END ) AS PIEL,
							 -- ===========================
							( CASE WHEN (	SELECT COUNT(colkey) 
											FROM cccuthst_sql (NOLOCK)
											WHERE JOBNO = @VP_ORDEN
											AND LTRIM(RTRIM(lotno)) = RIGHT('000000' +  LTRIM(RTRIM(LOTNO)), 6)
											AND HIDENO = HIDE
										) > 0 THEN  '* ' + HIDE
									ELSE '  ' + HIDE END ) AS PIEL,
							 -- ===========================
					...
					FROM	RP_SC (NOLOCK)
					--LEFT JOIN CCCUTHST_SQL (NOLOCK) ON CCCUTHST_SQL.JOBNO = @VP_ORDEN
					--							AND	 ( CASE WHEN SUBSTRING(LTRIM(RTRIM(LOTNO)),1,1) = '0' THEN SUBSTRING(LTRIM(RTRIM(LOTNO)),2, 10) ELSE LTRIM(RTRIM(LOTNO)) END ) = LOT 
					--							AND HIDENO = HIDE
					WHERE	TAGNO = @PP_FOLIO
					ORDER BY LOT, HIDE

					-- ////SE OBTIENEN LOS MOVIMIENTOS DE EXTRAS Y DEVOLUCIONES DEL FOLIO//////////////////////////////////////////////////////////
					...
					SELECT TOP 1000
						...
						( CASE WHEN D_TIPO_PIEL_LOG = 'TRANSFERENCIA A FOLIO' AND FOLIO_ORIGEN <>  @PP_FOLIO THEN 'EXTRAS'
									WHEN D_TIPO_PIEL_LOG = 'CAMBIO COLOR' AND FOLIO_ORIGEN <>  @PP_FOLIO THEN 'EXTRAS'
									WHEN D_TIPO_PIEL_LOG = 'CAMBIO COLOR' AND FOLIO_ORIGEN =  @PP_FOLIO THEN 'DEVOLUCION'
					...
					INNER JOIN TIPO_PIEL_LOG (NOLOCK) ON PIEL_LOG.K_TIPO_PIEL_LOG = TIPO_PIEL_LOG.K_TIPO_PIEL_LOG
					WHERE (FOLIO_ORIGEN = @PP_FOLIO OR FOLIO_DESTINO = @PP_FOLIO)
					AND PIEL_LOG.K_TIPO_PIEL_LOG IN (5, 6, 9, 11) -- #5	TRANSFERENCIA A FOLIO	#6 DEVOLUCION	#9 FOLIO SCRAP # 11 CAMBIO COLOR (CUANDO SE GENERAM )
					ORDER BY K_PIEL_LOG, LOTE, PIEL ASC

					-- ///////SE CARGAN A LA TABLA QUE MOSTRARA EL RESULTADO///////////////////////////////////////////////////////
					INSERT INTO @VP_TBL_FOLIO
					SELECT D_TIPO_PIEL_LOG, LOTE, PIEL, SQF  FROM @VP_TBL_FOLIO_MOVIMIENTO_ORDEN ORDER BY D_TIPO_PIEL_LOG DESC
				END
			ELSE
				BEGIN
					-- ////SE OBTIENEN LAS PIELES CARGADAS EN EL FOLIO//////////////////////////////////////////////////////////
					INSERT INTO @VP_TBL_FOLIO
					SELECT	'ACTUAL',
							LTRIM(RTRIM(LOT))	AS LOTE, 
