...
AS
	...
	SELECT DISTINCT jobno, status, CONCAT('F', LTRIM(RTRIM(ccjobhdr_sql.colour))),
					...
					--=============================================
					ISNULL((	SELECT TOP 1 CONCAT([dbo].[CONVERT_INT_TO_DATE](CDATE), ' ',  [dbo].[CONVERT_INT_TO_TIME](CTIME)) 
								FROM cccuthst_sql (NOLOCK)
								WHERE  cccuthst_sql.jobno = ccjobhdr_sql.jobno 
								ORDER BY COLKEY DESC), 0) AS F_ULTIMO_CORTE
					--=============================================
	...
	SELECT ID,				
		   ...
		   ( CASE WHEN PATTERNS_CORTADO > 0 AND PATTERNS_REQUERIDO > 0 THEN (PATTERNS_CORTADO * 100) / PATTERNS_REQUERIDO 
			...
			--=============================================
			( CASE WHEN (PIELES_FOLIO - PIELES_CORTADA) BETWEEN 0 AND 5 AND PATTERNS_CORTADO < PATTERNS_REQUERIDO THEN 1
					WHEN (PIELES_FOLIO - PIELES_CORTADA) < 0 AND PATTERNS_CORTADO < PATTERNS_REQUERIDO THEN 1
					WHEN (PIELES_FOLIO - PIELES_CORTADA) BETWEEN 6 AND 10 AND PATTERNS_CORTADO < PATTERNS_REQUERIDO THEN 2
					WHEN (PIELES_FOLIO - PIELES_CORTADA) > 10 AND PATTERNS_CORTADO < PATTERNS_REQUERIDO THEN 3
					WHEN PATTERNS_CORTADO >= PATTERNS_REQUERIDO THEN 4 END ) AS PRIORIDAD_EXTRAS,
			--=============================================
		   NETO_CORTADO,	
		   SQF_CORTADO,
		   CONVERT(VARCHAR(16), F_ULTIMO_CORTE) F_ULTIMO_CORTE
