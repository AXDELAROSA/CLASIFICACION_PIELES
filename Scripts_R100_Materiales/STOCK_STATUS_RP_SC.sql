...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PG_GET_RP_SC_LOTE_SIN_STOCK]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].[PG_GET_RP_SC_LOTE_SIN_STOCK]
GO

--	EXEC [PG_RN_VALIDA_USUARIO_MQU] 0,0,144

CREATE PROCEDURE [dbo].[PG_GET_RP_SC_LOTE_SIN_STOCK]
...
AS
	...
	WHILE @@FETCH_STATUS = 0
		BEGIN
			...
			SELECT @PP_LOCACION, @PP_COLOR, @VP_LOTE, COUNT(HIDE), SUM(CONVERT(DECIMAL(13,2), SQF)),@VP_SQF_LOC
			  FROM RP_SC 
			  WHERE LTRIM(RTRIM(COLOUR)) = @PP_COLOR 
